...
public:
  ...
  uintptr_t get_address() const { return this->m_address; }

  Pattern add(const ptrdiff_t offset) const { return Pattern(this->m_address + offset); }

  Pattern sub(const ptrdiff_t offset) const { return Pattern(this->m_address - offset); }

  Pattern rip(const ptrdiff_t offset = 0x03, const size_t length = 0x07) const;

  template <typename T> T as() { return reinterpret_cast<T>(this->m_address); }
...
public:
  ...
  uint32_t get_pid() { return this->m_pid; }
  ...
  template <typename T> T readv(void *address) {
    T value;
  ...
  template <typename T> T readv(uintptr_t address) noexcept {
    T buffer;
    this->read_raw(reinterpret_cast<void *>(address), &buffer, sizeof(T));
    return buffer;
  }

  template <typename T> T writev(uintptr_t address, T value) {
    WriteProcessMemory(this->m_handle, (void *)address, &value, sizeof(T), NULL);
    return value;
  }
...
private:
  ...
  bool read_raw(void *address, void *buffer, const size_t size) { return ReadProcessMemory(this->m_handle, reinterpret_cast<void *>(address), buffer, size, nullptr); }
